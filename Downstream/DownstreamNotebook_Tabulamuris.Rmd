...
nFeature_threshold <- c(300, 2200) # subset. Minimum and maximum number of features detected in a cell to be kept
...
```{r}
idents_TM <- as.data.frame(seurat.obj.filtered$cluster.ids)
idents_TM <- as.data.frame(cbind(rownames(idents_TM),idents_TM$`seurat.obj.filtered$cluster.ids`))
names(idents_TM)=c("CellBarcodes", "Cluster")
idents_TM[,1] <- substring(idents_TM[,1],11)
