...
tabulaMuris_anotatedTable_path<-"./Refs/10X_P8_15_anotatedTable.csv" # Make sure this file is obtained from the data of the same tissue
clusterIdents_path<-"./Refs/10X_P8_15_idents.csv"
```
...
```{r}
nCount_threshold <- c(500, 10000)  # subset. Minimum and maximum number of molecules detected in a cell to be kept
nFeature_threshold <- c(300, 2100) # subset. Minimum and maximum number of features detected in a cell to be kept
...
anotated_table %>% inner_join(tabulaMuris_anotatedTable, by="description") %>% select(gene.y, description, cluster.x, cluster.y, myAUC.x, avg_log2FC.x) %>% knitr::kable(col.names = c("Gene", "Description", "Cluster of project", "Ground truth cluster", "myAUC", "avg_log2FC"))
```
